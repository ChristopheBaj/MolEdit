...
class LoadModel:
    ...
    def __init__(self,
        ...
        if n_output_scalers == 0:
            print("ERROR:")
            print("Property scalers for the dataset are not found in:")
            print(scaler_dir)
            print("")
            print("*** LOADING ABORTED ***")
            raise utils.StopExecution
        if n_extra_scalers != 0:
            self.extra=True
            # Verify that the numbers of scalers are consistent
            if n_output_scalers!=n_extra_scalers:
                print("ERROR:")
                print("The number of the property scalers `*Scaler_Outputs*` and the")
                print("number of extra input `*Scaler_Extra*` do not match.")
                print("Scalers directory:")
                print(scaler_dir)
                print("")
                print("*** LOADING ABORTED ***")
                raise utils.StopExecution
        ...
        # Automatically assign the number of folds
        self.k_fold_number = n_output_scalers
            
        # Automatically assign the number of runs
        model_dir = self.train_dir + '/Models'
        n_models = len(glob.glob(model_dir + "/*"))
        if n_models%self.k_fold_number==0:
            self.n_runs = n_models//self.k_fold_number
            print("The number of runs is automatically set to " + str(self.n_runs))
        else:
            print("ERROR:")
            print("Cannot set up the number of runs automatically.")
            print("The number of models is not divisible by the number number")
            print("of scalers.")
