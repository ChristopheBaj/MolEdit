...
def bayopt_run(smiles, prop, extra, train_val_idx, smiles_concat, tokens, max_length, check_smiles, augmentation, hyper_bounds, hyper_opt, dense_depth, bo_rounds, bo_epochs, bo_runs, strategy, pretrained_model=None):
    ...
    def bayopt_func(params):
        ...
        for irun in range(bo_runs):
            ...
            if math.isnan(mse_valid): # treat diverging architectures (rare event)
                mse_valid = math.inf
            mse_valids.append(mse_valid)

        logging.info('Average best validation RMSE: {0:0.4f}'.format(np.mean(mse_valids)))

        return np.mean(mse_valids)
